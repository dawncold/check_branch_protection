...
@click.option('--repo-prefix', required=True, help='filter repos prefix')
@click.option('--branch', 'branch_to_check', default='master', help='branch to check', show_default=True)
@click.option('--setup', 'setup_protection', is_flag=True, help='setup basic branch protection (disable force push)')
...
def main(token, repo_prefix, branch_to_check, setup_protection, delete_protection):
    ...
    print(f'setup protection: {setup_protection}, delete protection: {delete_protection}')
